...
inline Force GetForce(CenterOfMass const& obj1, CenterOfMass const& obj2) {
    const double G = 2.95736e-4;
    
    const double rx = obj1.x - obj2.x;
    const double ry = obj1.y - obj2.y;
    
    const double r2 = rx*rx + ry*ry;
    const double r  = std::sqrt(r2);
    
    const double f = -G*obj1.mass*obj2.mass / r2 / r;
